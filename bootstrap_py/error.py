...
class DuplicateError:
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{self.span}: Duplicate definition of `{self.name}`:\n{code}\nAlready defined here:\n{defined_here_code}"
        )
...
def unknown_token(span: Span, token: str) -> Error:
    return SimpleError(span, f"Unknown token `{token}`")
...
def unexpected_token(span: Span, got: str, *expected: str) -> Error:
    ...
    return SimpleError(span, f"{prefix}{expected_names}, got `{got}`")
...
def duplicate_fn(name: str, span: Span, defined_here: Span) -> Error:
    return DuplicateError(name, span, defined_here)


def undefined_name(name: str, span: Span) -> Error:
    return SimpleError(span, f"Undefined name `{name}`")


def unexpected_type(expected: str, got: str, span: Span) -> Error:
    return SimpleError(span, f"Expected {expected}, got {got}")


def cascaded_error(span: Span, origin_message: str, originated_here: Span) -> Error:
    return CascadedError(span, origin_message, originated_here)


def wrong_number_of_args(span: Span, params: int, args: int, defined_here: Span) -> Error:
    return WithDefinitionError(span, f"Expected {params} arguments, got {args}", defined_here)


def type_not_assignable_from(span: Span, target: str, from_: str) -> Error:
    return SimpleError(span, f"Type `{from_}` is not assignable to type `{target}`")
